...
import type { Patient, Visit, Prescription, ExercisePlan, DoctorObservation } from "@/types";
import {
  ArrowLeft,
  Edit,
  Trash2,
...
export default function PatientDetailPage() {
  ...
  const [editingObservation, setEditingObservation] = useState<DoctorObservation | null>(null);
  ...
  const loadPatientData = async () => {
    try {
      const [patientData, visitsData, prescriptionsData, exercisePlansData, observationsData] =
        await Promise.all([
          getPatient(id!),
          getPatientVisits(id!),
  ...
  const openAddVisit = () => {
    ...
    setObservationForm({ diagnosis: '', examinationFindings: '', warningsAndPrecautions: '', treatmentPlan: '' });
    setMriFinding('');
    setXrayFinding('');
    setExerciseProtocol('');
  ...
  const openEditVisit = (v: Visit) => {
    ...
    if (obs) {
      ...
      setObservationForm(prev => ({ ...prev, examinationFindings: rest.join('\n') }));
    } else {
      setObservationForm({ diagnosis: '', examinationFindings: '', warningsAndPrecautions: '', treatmentPlan: '' });
      setMriFinding('');
      setXrayFinding('');
  ...
  const saveVisit = async () => {
    try {
      if (editingVisit) {
        ...
        if (visitForm.painSeverity !== '') updates.painSeverity = Number(visitForm.painSeverity);
        ...
        const existingObs = observations.find(o => o.visitId === editingVisit.id) || null;
        try {
          if (
            observationForm.diagnosis || combinedFindings ||
            observationForm.warningsAndPrecautions || observationForm.treatmentPlan
          ) {
            const obsPayload: any = {
              diagnosis: observationForm.diagnosis || undefined,
              examinationFindings: combinedFindings || undefined,
              warningsAndPrecautions: observationForm.warningsAndPrecautions || undefined,
        ...
        try {
          const existingPlan = exercisePlans.find(p => p.visitId === editingVisit.id) || null;
          const trimmed = exerciseProtocol.trim();
          if (trimmed) {
            if (existingPlan) {
              // Shallow update: replace first exercise name
              const updated = { ...existingPlan, exercises: [...(existingPlan.exercises || [])] };
              if (updated.exercises.length) updated.exercises[0] = { ...updated.exercises[0], name: trimmed };
              else updated.exercises = [{ name: trimmed, repetitions: '-', sets: '-', frequency: '-', duration: '-' }];
              ...
              await createExercisePlan({
                patientId: id!, patientName: patient!.fullName, visitId: editingVisit.id,
                exercises: updated.exercises,
                prescribedBy: user!.uid, prescribedByName: user!.name || user!.email,
                createdAt: Date.now(), updatedAt: Date.now(),
              } as any);
            } else {
              await createExercisePlan({
                patientId: id!, patientName: patient!.fullName, visitId: editingVisit.id,
                exercises: [{ name: trimmed, repetitions: '-', sets: '-', frequency: '-', duration: '-' }],
                prescribedBy: user!.uid, prescribedByName: user!.name || user!.email,
                createdAt: Date.now(), updatedAt: Date.now(),
              } as any);
            }
        ...
        if (visitForm.painSeverity !== '') visitPayload.painSeverity = Number(visitForm.painSeverity);
        if (visitForm.visitNotes) visitPayload.visitNotes = visitForm.visitNotes;
        ...
        if (
          ...
          try {
            ...
            if (observationForm.diagnosis) obsPayload.diagnosis = observationForm.diagnosis;
            if (combinedFindings) obsPayload.examinationFindings = combinedFindings;
            if (observationForm.warningsAndPrecautions) obsPayload.warningsAndPrecautions = observationForm.warningsAndPrecautions;
            if (observationForm.treatmentPlan) obsPayload.treatmentPlan = observationForm.treatmentPlan;
            await createDoctorObservation(obsPayload);
          } catch (obsErr) {
            console.warn('createDoctorObservation failed, visit created', obsErr);
            toast({ title: 'Saved with warnings', description: 'Visit saved, but diagnosis could not be saved due to permissions.', variant: 'default' });
          }
        }
        ...
        if (exerciseProtocol.trim()) {
          try {
            const planPayload: any = {
              ...
              visitId,
              exercises: [
                { name: exerciseProtocol.trim(), repetitions: '-', sets: '-', frequency: '-', duration: '-' }
              ],
            ...
            toast({ title: 'Saved with warnings', description: 'Visit saved, but exercise protocol could not be saved due to permissions.', variant: 'default' });
          }
        }
  ...
  const removeVisit = async (visitId: string) => {
    try {
      ...
      if (remainingVisits.length) {
        const latestVisit = [...remainingVisits].sort((a,b) => b.visitDate - a.visitDate)[0];
        complaint = latestVisit.chiefComplaint || '';
        dateTs = latestVisit.visitDate;
        const obs = remainingObs.find(o => o.visitId === latestVisit.id) || null;
        if (obs) {
          diagnosis = obs.diagnosis || '';
  ...
  const saveObservation = async () => {
    ...
    try {
      ...
      toast({ title: 'Error', description: 'Failed to update diagnosis & plan', variant: 'destructive' });
    }
  };
  ...
  const removeObservation = async (obsId: string) => {
    try {
      ...
      toast({ title: 'Error', description: 'Failed to delete diagnosis & plan', variant: 'destructive' });
    }
  };
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          {(() => {
            ...
            observations.forEach(o => { if (!obsByVisit[o.visitId]) obsByVisit[o.visitId] = o; });
            const exByVisit: Record<string, ExercisePlan> = {};
            exercisePlans.forEach(ex => { if (!exByVisit[ex.visitId]) exByVisit[ex.visitId] = ex; });

            const items = [...visits].sort((a, b) => b.visitDate - a.visitDate);
            if (!items.length) {
              return (
                <div className="text-center py-8 text-muted-foreground">
                  <p>No records yet. Start by adding a visit, diagnosis or exercises.</p>
                </div>
              );
            }
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <div>
                          ...
                          <div className="mt-2">
                            {o?.diagnosis && <div className="text-sm"><span className="font-medium">📝 My diagnosis:</span> {o.diagnosis}</div>}
                            ...
                            {(() => {
                              ...
                              findings.split('\n').forEach((line) => {
                                const t = line.trim();
                                if (/^MRI\s*:/.test(t)) mriText = t.replace(/^MRI\s*:\s*/i, '');
                                else if (/^X-?ray\s*:/.test(t)) xrayText = t.replace(/^X-?ray\s*:\s*/i, '');
                            ...
                            {o?.warningsAndPrecautions && <div className="mt-1 text-sm">🛑 Precautions: {o.warningsAndPrecautions}</div>}
                            {o?.treatmentPlan && <div className="mt-1 text-sm">✳️ Rx plan: {o.treatmentPlan}</div>}
                          </div>
                          <div className="mt-2">
                            <div className="text-sm">❇️ Exercise protocol: {ex?.exercises?.[0]?.name || ''}</div>
                          </div>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <Button size="sm" variant="outline" onClick={() => openEditVisit(v)}>Edit</Button>
                        <Button size="sm" variant="destructive" onClick={() => removeVisit(v.id)}>Delete</Button>
                      </div>
                    </div>
                  </div>
                )})}
      ...
      <Dialog open={visitDialogOpen} onOpenChange={setVisitDialogOpen}>
        <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-hidden flex flex-col">
          <DialogHeader>
            <DialogTitle>{editingVisit ? 'Edit Visit' : 'Add Visit'}</DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-2 overflow-y-auto pr-2" style={{ maxHeight: '65vh' }}>
            <div className="grid gap-2">
              <Label htmlFor="vdate">Date</Label>
              <Input id="vdate" type="date" value={visitForm.visitDate} onChange={(e) => setVisitForm({ ...visitForm, visitDate: e.target.value })} />
            </div>
            <div className="grid gap-2">
            ...
            <div className="grid gap-2">
              ...
              <Input type="number" min={0} max={10} value={visitForm.painSeverity} onChange={(e) => setVisitForm({ ...visitForm, painSeverity: e.target.value })} />
            </div>
            <div className="grid gap-2">
              <Label>Notes</Label>
              <Textarea value={visitForm.visitNotes} onChange={(e) => setVisitForm({ ...visitForm, visitNotes: e.target.value })} />
            </div>
            <div className="pt-2 border-t" />
            <div className="grid gap-2">
              <Label>📝 My diagnosis</Label>
              <Textarea value={observationForm.diagnosis} onChange={(e) => setObservationForm({ ...observationForm, diagnosis: e.target.value })} />
            </div>
            <div className="grid gap-2">
              <Label>▫️MRI Finding</Label>
              <Textarea value={mriFinding} onChange={(e) => setMriFinding(e.target.value)} />
            </div>
            <div className="grid gap-2">
              <Label>▫️ X-ray finding</Label>
              <Textarea value={xrayFinding} onChange={(e) => setXrayFinding(e.target.value)} />
            </div>
            <div className="grid gap-2">
              <Label>🛑 Precautions</Label>
              <Textarea value={observationForm.warningsAndPrecautions} onChange={(e) => setObservationForm({ ...observationForm, warningsAndPrecautions: e.target.value })} />
            </div>
            <div className="grid gap-2">
              <Label>✳️ Rx plan</Label>
              <Textarea value={observationForm.treatmentPlan} onChange={(e) => setObservationForm({ ...observationForm, treatmentPlan: e.target.value })} />
            </div>
            <div className="grid gap-2">
              <Label>❇️ Exercise protocol</Label>
              <Textarea value={exerciseProtocol} onChange={(e) => setExerciseProtocol(e.target.value)} />
            </div>
          </div>
          <DialogFooter>
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="grid gap-4 py-2">
            ...
            <div className="grid gap-2">
              ...
              <Textarea value={observationForm.examinationFindings} onChange={(e) => setObservationForm({ ...observationForm, examinationFindings: e.target.value })} />
            </div>
            <div className="grid gap-2">
              <Label>Precautions</Label>
              <Textarea value={observationForm.warningsAndPrecautions} onChange={(e) => setObservationForm({ ...observationForm, warningsAndPrecautions: e.target.value })} />
